...
object InOrderHBMPCArbiter {
	val numMemoryPorts = 4
}

class InOrderHBMPCArbiter(
		...
		numMemoryPorts:      Int=InOrderHBMPCArbiter.numMemoryPorts
	...
	val inReqWithFullAddrs = Wire(Vec(numReqHandlers, DecoupledIO(UInt(fullTagWidth.W))))
	for (i <- 0 until numReqHandlers) {
		inReqWithFullAddrs(i).valid := io.inReq(i).valid
		io.inReq(i).ready := inReqWithFullAddrs(i).ready
		if (numReqHandlers > 1) {
			inReqWithFullAddrs(i).bits := Cat(io.inReq(i).bits, i.U(handlerAddrWidth.W))
		} else {
	...
	// val scrambledInputs = {
	// 	if (numReqHandlers > 1) {
	// 		Scrambler(inReqWithFullAddrs(0).bits.cloneType, inReqWithFullAddrs, numMemoryPorts)
	// 	} else {
	// 		val inputArbiter = Module(new ResettableRRArbiter(UInt(tagWidth.W), numReqHandlers))
	// 		for (i <- 0 until numReqHandlers) inputArbiter.io.in(i) <> io.inReq(i)
	// 		Vec(1, inputArbiter.io.out)
	...
	val memInterfaceManagers = Array.fill(numMemoryPorts)(
	...
	val addrCrossbar = Module(new OneWayCrossbarGeneric(
		...
		numReqHandlers,
		numMemoryPorts,
		(addrIn: UInt) => addrIn(channelAddrWidth + hbmChannelWidth - offsetWidth - 1, hbmChannelWidth - offsetWidth),
