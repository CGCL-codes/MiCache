...
class ReorderBufferAXINoProfiling(
    addrWidth: Int=ReorderBufferAXI.addrWidth,
    dataWidth: Int=ReorderBufferAXI.dataWidth,
    idWidth: Int=ReorderBufferAXI.idWidth
) extends Module {
    val io = IO(new Bundle {
        val in = new AXI4FullReadOnly(UInt(dataWidth.W), addrWidth, 0)
        val out = Flipped(new AXI4FullReadOnly(UInt(dataWidth.W), addrWidth, idWidth))
    })
...
object ReorderBufferGen extends App {
    val addrWidth = 32
    val dataWidth = 32
    val idWidth = 13
	chisel3.Driver.execute(
		args,
		() => new ReorderBufferAXINoProfiling(addrWidth, dataWidth, idWidth)
	)
}
