...
class AXI4Stream[T <: Data](dataType: T) extends Bundle {
    val TVALID = Input(Bool())
    val TREADY = Output(Bool())
    val TDATA  = Input(dataType)
    val TSTRB  = Input(UInt((dataType.getWidth / 8).W))
    val TKEEP  = Input(UInt((dataType.getWidth / 8).W))
    val TLAST  = Input(Bool())
    override def cloneType = (new AXI4Stream(dataType)).asInstanceOf[this.type]
}
